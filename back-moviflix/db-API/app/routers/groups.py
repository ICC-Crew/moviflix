...
@router.get("", dependencies=[Depends(JWTBearer())], response_description="List all groups in DB",response_model=List[Group])
async def get_groups(db = Depends(get_database)): 
    groupList = await fetch_groups(db)
    return groupList

@router.get("/{groupId}", dependencies=[Depends(JWTBearer())], response_description="Find a group with its MongoDB ID",response_model=Group)
...
@router.patch("", dependencies=[Depends(JWTBearer())], response_description="Update an existing group of the DB")
